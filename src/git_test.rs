...
mod get_root_directory_path_tests {
    ...
    fn uses_git_rev_parse_top_level_command() {
        let exp = "/usr/me/foo";
        let target_dir = "";
        let run_command = |cmd: &str, dir: Option<&str>| {
            if cmd == "git rev-parse --show-toplevel" && dir == Some(target_dir) {
                Ok(String::from(exp))
            } else {
                Ok(String::from(""))
            }
...
mod get_hooks_directory_tests {
    ...
    fn uses_git_hooks_path_command() {
        ...
        let target_dir = "";
        let run_command = |cmd: &str, dir: Option<&str>| {
            if cmd == "git rev-parse --git-path hooks" && dir == Some(target_dir) {
                Ok(String::from(exp))
            } else {
                Ok(String::from(""))
            }
...
mod setup_hooks_tests {
    ...
    fn errors_when_hooks_directory_unknown() {
        ...
        let run_command = |_cmd: &str, _dir: Option<&str>| Err(String::from(""));
    ...
    fn errors_when_hook_write_fails() {
        let run_command =
            |_cmd: &str, _dir: Option<&str>| Ok(String::from("/usr/repos/foo/.git/hooks"));
    ...
    fn creates_all_hooks() {
        ...
        let run_command = |_c: &str, _d: Option<&str>| Ok(String::from(git_hooks));
